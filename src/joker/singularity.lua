SMODS.Joker {
  ...
  pos = { x = 6, y = 5 },   --position in atlas, starts at 0, scales by the atlas" card size (px and py): {x = 1, y = 0} would mean the sprite is 71 pixels to the right
  config = {
    extra = {
      level = 1
    }
  ...
  calculate = function(self, card, context)
    if context.using_consumeable and context.consumeable.ability.set == "Planet" then
      ...
      update_hand_text({ sound = "button", volume = 0.7, pitch = 0.8, delay = 0.3 },
        { handname = "All Hands +1", chips = "...", mult = "...", level = "" })
